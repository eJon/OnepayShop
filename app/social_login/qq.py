...
@qq_bp.route('/user_info')
def get_user_info():
    if 'qq_token' in session:
        data = update_qq_api_request_data()
        # print data
        user_info = qq.get('/user/get_user_info', data=data)
        user = utils.login_or_create_union_user(data["openid"], json_to_dict(user_info.data), "qq")
        # return jsonify(user.get_display_data())
        return redirect(url_for('home'))
    return redirect(url_for('login'))


@qq_bp.route('/login/qq')
def qq_login():
    # print url_for('.authorized')
    return qq.authorize(callback=url_for('.authorized', _external=True))


@qq_bp.route('/login/authorized')
def authorized():
    # if session.get('qq_openid'):
    ...
    session['qq_token'] = (resp.get('access_token'), '')
    if not session['qq_token'][0]:
        return redirect(url_for('home'))
